...
class Decoder(BaseModule):
    ...
    def __init__(self, code_length, deepest_shape, output_shape):
        ...
        # FC network
        self.tdl = nn.Sequential(
            TemporallySharedFullyConnection(in_features=code_length,
                                            out_features=(dc * dh * dw)),
            activation_fn
        )
    ...
    def forward(self, x):
        ...
        h = self.conv(h)
        # TODO 内存由2300->9700暴涨的地方，并且内存不回收也是bug
        o = h

        return o

# just Encoder
class LSAUCSD_deepSVDD(BaseModule):
    """
    ...
    def __init__(self, input_shape, code_length):
        ...
        """
        super(LSAUCSD_deepSVDD, self).__init__()
