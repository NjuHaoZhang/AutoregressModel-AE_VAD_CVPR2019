...
# For Testing
# class UCSDPed1(VideoAnomalyDetectionDataset):
#     pass

# torch.device("cuda:" + "3")

class UCSDPed1_deepSVDD(VideoAnomalyDetectionDataset):
    """
    Models UCSD Ped1 dataset for video anomaly detection.
    """
    def __init__(self, path, exrta_paras_dict):
        ...
        """
        super(UCSDPed1_deepSVDD, self).__init__()

        self.path = join(path, 'UCSDped1')
    ...
    def load_test_sequence_gt(self, video_id):
        # 根据 .m file 直接返回 a np.ndarray, with shape (n_frames,).
        sequence_dir = join(self.test_dir, video_id)
        # By HaoZhang, for UCSD/pde1 or ped2, video_id: TestXXX(XXX：001~012)
        img_list = sorted(glob(join(sequence_dir, '*.tif')))  # 当前 vid下面所有tif
        ...
        #
        dataset = "UCSD_ped1"
        video_id = video_id[-2:] # video_id: TestXXX(XXX：001~012)
        gt_path = os.path.join(self.test_dir, 'frame_level_gt/gt_{}_vid{}.txt'.format(dataset, video_id))
        gt_vid_raw = np.loadtxt(gt_path)
        gt_vid = np.zeros(frame_num)
        try:
            for event in range(gt_vid_raw.shape[0]): # 多个时间段
                start = int(gt_vid_raw[event, 0]) - 1
                end = int(gt_vid_raw[event, 1])
                gt_vid[start:end] = 1
        except IndexError: # 一个时间段
            start = int(gt_vid_raw[0]) -1
    ...
    def __repr__(self):
        return f'UCSD Ped1 (video id = {self.cur_video_id})'

# 测试的小代码段
def test_test_load_test_ids():
    dataset = UCSDPed1_deepSVDD(path='/home/zh/Papers_Code/CVPR2019_pytorch_VAD'
...
def test_load_test_sequence_gt():
    dataset = UCSDPed1(path='/home/zh/paper_codes/novelty-detection/'
    ...
    #
    # video_id = 'Test036'
    # sequence_dir = join(dataset.test_dir, video_id)
    # # By HaoZhang, for UCSD/pde1 or ped2, video_id: TestXXX(XXX：001~012)
    # img_list = sorted(glob(join(sequence_dir, '*.tif')))  # 当前 vid下面所有tif
    # frame_num = len(img_list)
    # gt = np.zeros(frame_num)
    # # 15:108
    # gt[14:108] = 1
    # ret = dataset.load_test_sequence_gt(video_id)
    # print("ret: ", ret)
    # print("gt: ", gt)
    # print("ret==gt: ", ret==gt)
    # #
    ...
    # 在做几组单元测试，就是手工获得ret 与 本程序返回的ret 做 比较
    pass

#####################################################################################

# For Training by HaoZhang
class UCSDPed1_deepSVDD_TRAIN(VideoAnomalyDetectionDataset):
    """
    ...
    def __init__(self, path, exrta_paras_dict):
        ...
        :param path: The folder in which UCSD is stored.
        """
        super(UCSDPed1_deepSVDD_TRAIN, self).__init__()
...
def test_TRAIN():
    dataset = UCSDPed1_deepSVDD_TRAIN(path='/home/zh/Papers_Code/CVPR2019_pytorch_VAD'
                                  '/novelty-detection/data/UCSD_Anomaly_Dataset.v1p2')
